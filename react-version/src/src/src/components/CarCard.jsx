const CarCard = ({ car }) => {
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200 hover:scale-105 transition-transform duration-300">
      <img src={`/images/${car.img}`} alt={car.model} className="w-full h-48 object-cover" />
      <div className="p-4">
        <h3 className="text-xl font-bold uppercase">{car.brand} {car.model}</h3>
        ...
        <div className="mt-4 flex justify-between items-center">
          ...
          <span className="bg-green-100 text-green-700 px-2 py-1 rounded text-sm">{car.status}</span>
        </div>
      </div>
    </div>
  );
